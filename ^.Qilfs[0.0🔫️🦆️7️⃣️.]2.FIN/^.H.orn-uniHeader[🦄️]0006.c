...
int STD_debug_Print2Txt_H(char *snprintfOut){


  int fd = open("x.x_debug_out.txt", O_RDWR | O_APPEND | O_CREAT, 0777);//❎️👾️
//read(fd, local_BlockBuffer_0, stdBlockSize_H);

   // write2Position_0000(0,local_BlockBuffer_0,  local_BlockCountBinary ,STD_txtFS_SB_fieldlength_bin_H );


write(fd, snprintfOut ,strlen(snprintfOut)); //
close(fd); 
...
int Std_inc_FD_POS_count_H(char *superBLock_filename_H_1, int position_arg,int bytesize_arg){
printf("in task...🧟️➕️➕️\n");
int fd;
char *local_BlockBuffer_0 = malloc(stdBlockSize_H); //where FREE()?
char *local_BlockCountBinary = malloc(bytesize_arg);

/////////////////////////
int realInodeCount = Std_read_FD_POS_count_H(superBLock_filename_H_1, position_arg,bytesize_arg); 

realInodeCount++;

converttobinaryMalloc2(realInodeCount,  local_BlockCountBinary , bytesize_arg); 
...
close(fd); 
    ...
    write2Position_0000(position_arg,local_BlockBuffer_0,  local_BlockCountBinary ,STD_txtFS_SB_fieldlength_bin_H );
...
//int newReadInodeCount =  Std_read_iblox_count_H(superBLock_filename_H_1); 

//int 
realInodeCount = Std_read_FD_POS_count_H(superBLock_filename_H_1, position_arg,bytesize_arg); 

return realInodeCount ; 
}//🎁️⚖️
//🎁️⚖️🎁️⚖️🎁️⚖️🎁️⚖️🎁️⚖️🎁️⚖️🎁️⚖️🎁️⚖️🎁️⚖️
int Std_read_FD_POS_count_H(char *superBLock_filename_H_1, int position_arg,int bytesize_arg){
// 💳️should we just MAKE a generic "file position reader?"⁉️
//Std_inc_iblox_count_H() < ALSO we 'DO "THIS"' ALOT (3xnow...)
// ITS NOT UR FAULT BUT NOW THAT U KNOW NOT HAVING A *
// STD INC++ SEEMS SILLY AS WELL DOESN'T IT ? *
// I KNOW THESE SEEM "TRIVIAL / WASTE OF TIME
// BUT THEY ACTUALLY MAKE FUTURE CODE EASIER / CLEANER...💳️
// OFC FOR NOW U CAN JUST 'GET IT DONE' 
// CUZ ITS 'ONE OF THOSE BIG DEAL THINGS'
// THAT ONCE THRU CLEAN UP IS EASIER...

// SO FOR THIS JUST PUT INODE COUNT AT BYTE 3  ALSO W/E
// AND WE CAN MOVE THAT LATER 2 . THE COMMENTS CAN STAY TILL THEN
//✳️
printf("read_inode_count_H)🧟️➕️➕️❗️\n");
int fd;
char *local_BlockBuffer_0 = malloc(stdBlockSize_H);
...
char *local_iNodeCountBinary = malloc(bytesize_arg);

printf("local_iBlockCountBinary)🧟️➕️➕️☘️ _inode_count_H \n");

readFrom_Position(position_arg,local_BlockBuffer_0 ,local_iNodeCountBinary,STD_txtFS_SB_fieldlength_bin_H );

//int blockCount; 
 int iNodeCount = bin2dec(local_iNodeCountBinary); 
// iblockCount-- ; // u may wanna leave this , it maybe because 
			// 0-1 = -1 ; whiches 2 confusing right now
			// till we have better inspection tools ?4.24.24
return iNodeCount;
...
//🦾️ inode fix arm
//👨🏽‍🚀️
// this is returning wrong number {i have 2 keep subtracting 1 from return}. lets fix it later.
// im gonna make the code work (inodes b4 i clean it up w/e ) just seems simpler 2 fix.👨🏽‍🚀️ur good bud.<3 👨🏽‍🚀️
//int Std_read_inode_count_H(char *superBLock_filename_H_1){
int Std_read_iblox_count_H(char *superBLock_filename_H_1){
int fd;
...
char *local_iBlockCountBinary = malloc(STD_txtFS_SB_fieldlength_bin_H);
//char *local_BlockCountBinary = malloc(16);
printf("local_iBlockCountBinary)🧟️➕️➕️☘️ _inode_count_H \n");

readFrom_Position(0,local_BlockBuffer_0 ,local_iBlockCountBinary,STD_txtFS_SB_fieldlength_bin_H );

//int blockCount; 
 int iblockCount = bin2dec(local_iBlockCountBinary); 
// iblockCount-- ; // u may wanna leave this , it maybe because 
			// 0-1 = -1 ; whiches 2 confusing right now
			// till we have better inspection tools ?4.24.24
...
// what did u want 2 return instead? 
// the inode count...
...
//🔧️🔧️🔧️🔧️🔧️🔧️🔧️🔧️🔧️🔧️🔧️🔧️
//🏴️🏴️🏴️🏴️🏴️🏴️🏴️
int Std_inc_iblox_count_H(char *superBLock_filename_H_1){
printf("in task...🧟️➕️➕️\n");
int fd;
char *local_BlockBuffer_0 = malloc(stdBlockSize_H); //where FREE()?
char *local_BlockCountBinary = malloc(16);
...
/////////////////////////
int blockCount =  Std_read_iblox_count_H(superBLock_filename_H_1); 
...
int read_inode_count = Std_read_iblox_count_H(std_superBLockFilename_H);
printf("post(Std_read_iblox_count_H)🧟️➕️➕️\n");
...
printf("Std_inodeCountInt 4 inode -1 🧟️= %d \n", Std_inodeCountInt );
...
int read_block_count = Std_read_iblox_count_H(superBLock_filename_H_1);
...
//🦾️u probably wanna remove the "loop from char count , cuz we wont have one...
...
//char stdBlockBuffer_1[256];
  //char local_AsciiBuffer_1[16];
...
/////////📌️
//char stdBlockBuffer_1[256]; //null term writes? 🏚️
//(now it thinks its stack smashing u probably oughta null term somewhere🏚️
// also this causes it 2 write "binary str8 as a file into homedir)
/////////////////////
...
///////☄️☄️☄️☄️☄️☄️☄️☄️☄️☄️☄️☄️☄️☄️(these were added later , could be wrong
...
/*
 char status_0[1024];
  snprintf(status_0, sizeof(status_0), "🦾️ = %d,%d \n",localDropCount, global_iblox_ct );/// ,"🦾️ = %s,%s  \n",localDropCount,global_iblox_ct);

   STD_debug_Print2Txt_H(status_0);
  */

int fullPosition =  STD_writeToInodes_H( filename_arg,local_AsciiBuffer_1, i , global_iblox_ct);

global_iblox_ct = Std_read_iblox_count_H(std_superBLockFilename_H);
// we dont have 2 minus -1 here but do in 🦾️
...
return 0 ; 
  	...
  	////////////////////////////////////////////////🌏️
 //🌏️global inode count SHOULDNT RESET ! Pls pay attention 2 that
 int STD_writeToInodes_H(char *filename_arg, char *local_AsciiBuffer_1, int iCount , int global_iblox_ct){
 
 //////////////////////////////////////////////////🖨️🖨️
 //need inode count as well? sumthin aint rite
  ...
  char status_4[1024];
   // int int_0 = 411 ;
    //
   // char *char_1 = "🖨️🦄️\n"; 
   snprintf(status_4, sizeof(status_4)  ,"global_iblox_ct = %d  \n",global_iblox_ct);
 //  printf("🖨️checkprinter2= %s🖨️\n",status);
   STD_debug_Print2Txt_H(status_4);
    ///////////////////////////////////////////////🖨️
 ...
 ///////////////////////////////////////////////🖨️
  ...
  char status_2[1024];
   // int int_0 = 411 ;
    //
   // char *char_1 = "🖨️🦄️\n"; 
   snprintf(status_2, sizeof(status_2)  ,"iCount = %d  \n",iCount);
 //  printf("🖨️checkprinter2= %s🖨️\n",status);
   STD_debug_Print2Txt_H(status_2);
    ///////////////////////////////////////////////🖨️
   ///////////////////////////////////////////////🖨️
  char status_3[1024];
   // int int_0 = 411 ;
    //
   // char *char_1 = "🖨️🦄️\n"; 
   snprintf(status_3, sizeof(status_3)  ,"local_AsciiBuffer_1 = %s  \n",local_AsciiBuffer_1);
 //  printf("🖨️checkprinter2= %s🖨️\n",status);
   STD_debug_Print2Txt_H(status_3);
   ///////////////////////////////////////////////🖨️
 ...
 int nodeLimit = ((stdBlockSize_H / stdKbdAsciiSize_H) * global_iblox_ct) -1;
 ...
 if(iCount  > nodeLimit ){
   ...
   snprintf(status_4, sizeof(status_4)  ,"met nodeLimit ! 🎺️ \n");
 //  printf("🖨️checkprinter2= %s🖨️\n",status);
   STD_debug_Print2Txt_H(status_4);

//if((i * stdKbdAsciiSize_H) > (stdBlockSize_H * global_iblox_ct) -1 ){

//📌️ 📌️ 📌️ 📌️ 📌️ 📌️ 📌️ 📌️ 📌️ 4.12.24

//printf("(i * stdKbdAsciiSize_H) > (stdBlockSize_H * global_iblox_ct) -1 );%d, %d🏈️\n",(i * stdKbdAsciiSize_H), (stdBlockSize_H * global_iblox_ct) -1 );

global_iblox_ct = stdIncInodeAddBloc_H(filename_arg) + 1 ;
...
int subSum = (global_iblox_ct -1 )  * (stdBlockSize_H  )  ;

//int fullSum = (localDropCount * stdKbdAsciiSize_H ) - subSum ;
int fullSum = (iCount * stdKbdAsciiSize_H ) - subSum ;
...
//if (fullSum == -stdBlockSize_H){
if (fullSum == -256|| fullSum == -512){
fullSum = 0;
}
//🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️🦾️❤️‍🩹️
  write2Position_0000(fullSum ,stdBlockBuffer_1,local_AsciiBuffer_1,stdKbdAsciiSize_H); 
 ...
 ///////////////////////////////////////////////🖨️
  ...
  char status_1[1024];
   // int int_0 = 411 ;
    //
   // char *char_1 = "🖨️🦄️\n"; 
   snprintf(status_1, sizeof(status_1)  ,"fullSum = %d  \n",fullSum);
 //  printf("🖨️checkprinter2= %s🖨️\n",status);
   STD_debug_Print2Txt_H(status_1);
    ///////////////////////////////////////////////🖨️
   ///////////////////////////////////////////////🖨️
  char status_0[1024];
   // int int_0 = 411 ;
    //
   // char *char_1 = "🖨️🦄️\n"; 
   snprintf(status_0, sizeof(status_0)  ,"stdBlockBuffer_1 = %s  \n",stdBlockBuffer_1);
 //  printf("🖨️checkprinter2= %s🖨️\n",status);
   STD_debug_Print2Txt_H(status_0);
   ///////////////////////////////////////////////🖨️
...
//////////
//free(stdBlockBuffer_1);
return fullSum ;// global_iblox_ct ;
 }	
  		////////🌏️
